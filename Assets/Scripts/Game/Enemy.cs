...
public class Enemy : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        var nearestPlayer = GameManager.Instance.NearestPlayer(position);
        if (nearestPlayer is not null)
        {
            var direction = (nearestPlayer.transform.position - position).normalized;
            var rotation = Quaternion.LookRotation(direction, Vector3.forward);
            
            // Calculate size of a step
            var step = direction * (speed / 100);
            // Change rotation and move by a step
            mRigidBody.MoveRotation(rotation);
            mRigidBody.MovePosition(position + step);
        }
    }
